...
# empty_matrix = matrix(0, ncol(dt.sampled_balanced), 26)
# empty_list = list()
# for(i in 1:length(dt.sampled  _balanced)){
#   print(paste('processing: ', i, '/16000', sep=""))
#   pos_test = pos(dt.sampled_balanced$text[i]
#                  , zero.replace = 0
#                  , percent = T)
#   pos_prop = pos_test$POSprop
#   pos_prop$Filename = dt.sampled_balanced$Filename[i]
#   dt.sampled_balanced
...
for(i in seq_batch){
  ...
  pos_test = pos(dt.sampled_balanced$text[new_index]
                 , zero.replace = 0
                 , percent = T)
  pos_prop = pos_test$POSprop
  pos_prop$Filename = dt.sampled_balanced$Filename[new_index]
...
dt.data_pos = merge(dt.sampled_balanced, pos_proportions, by = 'Filename')

dt.data_pos = dt.data_pos[, -3]

save(dt.data_pos
     , file='/Users/bennettkleinberg/Dropbox/workshop_eurocss/hackathon_data/features/eurocss_mediadata_POS.RData')
